...
class CartDetailsViewModel extends Cubit<CartStates> {
 CartDetailsViewModel({required this.cartDetailsUseCase,required this.deleteItemFromCartUseCase,
 ...
 double getCount(String productId){
  double count=0;
  var list=productList!.where((element) {
   return element.product!.id==productId;
  },
  ).toList();
 ...
 Future<void> deleteItemFromCart(String productId,BuildContext context) async {
  ...
  either.fold((error){
   ...
   productList=response.data!.products;
   cart=response;
   ShopTabViewModel.get(context).changeNumOfCartItems(response.numOfCartItems!.toInt());

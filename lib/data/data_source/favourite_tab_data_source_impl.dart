...
import 'package:ecommerce/data/model/favourite_tap/GetAllWishListResponseDto.dart';
...
class FavouriteTabDataSourceImpl implements FavouriteTabDataSource {
  ...
  @override
  Future<Either<Failures, AddToWatchListResponseDto>> addToWatchList(
    ...
    try {
      ...
      if (checkConnection.contains(ConnectivityResult.mobile) ||
        ...
        var response = await apiManager.postData(EndPoint.addToWatchList,
            data: {'productId': productId}, headers: {'token': token});
        var addToWatchListResponse =
            AddToWatchListResponseDto.fromJson(response.data);
        if (response.statusCode! >= 200 && response.statusCode! < 300) {
          return Right(addToWatchListResponse);
        } else {
          return Left(
              ServerError(errorMessage: addToWatchListResponse.message!));
        }
      } else {
    ...
    try {
      ...
      if (checkConnection.contains(ConnectivityResult.mobile) ||
        ...
        var response = await apiManager
            .getData(EndPoint.addToWatchList, headers: {'token': token});
        var getWatchListResponse =
            GetAllWishListResponseDto.fromJson(response.data);
        if (response.statusCode! >= 200 && response.statusCode! < 300) {
